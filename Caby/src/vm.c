...
static void runtime_error(vm_t* vm, const char* str, ...) {
    ...
    size_t loc_idx = range_between(f->bc.data,  vm->ip);
...
static enum interpret_result interpret_ins(vm_t* vm, u8 ins) {
    switch (ins) {
        ...
        case OP_VAR_GLOBAL: {
            ...
            if (!new_v) {
                runtime_error(vm, "Error: Variable '%s' is already defined.\n", name->data);
                return INTERPRET_ERROR;
            }
            break;
        }
        case OP_GET_GLOBAL: {
            ...
            if (!table_get(&vm->globals, name_obj, &val)) {
                runtime_error(vm, "Error: Access to undefined variable '%s'.\n", name->data);
                return INTERPRET_ERROR;
            }
            push(vm, val);
            break;
        ...
        case OP_SET_GLOBAL: {
            ...
            if (table_set(&vm->globals, name_obj, v)) {
                runtime_error(vm, "Global variable '%s' is not defined!\n", name->data);
                return INTERPRET_ERROR;
            }
            break;
        }
        ...
        default:
            runtime_error(vm, "Unknown instruction 0x%x! Skipping...\n", ins);
    }
