...
mod parser_tests {
    ...
    fn conditional_test() {
        ...
        assert!(TopLevelParser::new().parse("if (1 <= 2) { 1 } else { 2 }").is_ok());
        assert!(TopLevelParser::new().parse("if(1) 1").is_err());
        assert!(TopLevelParser::new().parse("def foo(a) = if (1 < 2) {1} else {2}").is_ok());
        assert!(TopLevelParser::new().parse("def foo(a) = if (1 < 2) {1} else {2}; foo(1);").is_ok());
    }
